...
try:
    ads = get_ads_performance()
    roas_df = get_roas()

    if ads.empty:
        st.info("No hay datos de campañas todavia. El pipeline los cargará mañana.")
        st.stop()
    ...
    with col4:
        avg_ctr = ads['ctr'].mean()
        st.metric("CTR Promedio", f"{avg_ctr:.2f}%")
    with col5:
        avg_roas = roas_df['roas'].mean() if not roas_df.empty else 0
        color = "normal" if avg_roas >= 3 else "inverse"
        st.metric("ROAS Promedio", f"{avg_roas:.2f}x", delta="Bueno" if avg_roas >= 3 else "Bajo", delta_color=color)
    ...
    fig1.add_trace(go.Bar(
        x=roas_df["date"], y=roas_df["ad_spend"],
        name="Gasto en Ads", marker_color="#f8a4d0",
    ))
    fig1.add_trace(go.Bar(
        x=roas_df["date"], y=roas_df["revenue"],
        name="Revenue", marker_color="#e91e8c",
    ))
    fig1.add_trace(go.Scatter(
        x=roas_df["date"], y=roas_df["roas"],
        name="ROAS", yaxis="y2", mode="lines+markers",
        line=dict(color="#9c1463", width=2),
    ))
    fig1.update_layout(
        barmode="group",
    ...
    with col_left:
        ...
        fig2 = px.bar(
            camp, x="spend", y="campaign_name", orientation="h",
    ...
    with col_right:
        ...
        fig3 = px.bar(
            ctr_camp, x="ctr", y="campaign_name", orientation="h",
            labels={"ctr": "CTR (%)", "campaign_name": "Campaña"},
            color_discrete_sequence=["#9c1463"],
            text="ctr",
        )
